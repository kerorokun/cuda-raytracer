...
__global__ void calc_pixel(Vec *out, int samps) {
    ...
    // Create the curandstate to use later
    curandState state;

    int i = (h-y-1) * w + x;
    curand_init(x, y, x*y*w*h, &state);
...
int main(int argc, char *argv[]) {
    ...
    Vec *host_out;
    //host_out = (Vec *)malloc(sizeof(Vec) * w * h);
    cudaHostAlloc((void**) &host_out, sizeof(Vec) * w * h, cudaHostAllocDefault);

    Vec *device_out;
    cudaMalloc((void **) &device_out, sizeof(Vec) * w * h);

    printf("This is Chris's pinned memory optimization.\n");

    printf("Render starting!\nBlock size is %i\n", int(BLOCK_SIZE));
    dim3 grid(ceil(1024/BLOCK_SIZE), ceil(768/BLOCK_SIZE), 1);
