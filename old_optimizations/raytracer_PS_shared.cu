...
__global__ void calc_pixel(Vec *out, int samps, Ray cam, Vec cx, Vec cy) {
    ...
    int linear_i = blockIdx.x * blockDim.x + threadIdx.x;
    int tx       = linear_i / 4;
    int mod_4    = linear_i % 4;
    int sy       = mod_4 / 2;
    int sx       = mod_4 % 2;
    ...
    int start    = blockDim.x * blockIdx.x / 4;
    int offset   = threadIdx.x + (start) % w;
    int offset_2 = threadIdx.x + start;

    int t = threadIdx.x / 4;
    int i = (h-y-1) * w + (offset);

    if (threadIdx.x < BLOCK_SIZE) {
        curand_init(offset_2, 0, 0, &states[threadIdx.x]);
    }
    __syncthreads();
    ...
    for (int s = 0; s < samps; s++) {
        double r1 = 2*curand_uniform(&states[t]), dx=r1<1 ? sqrt(r1)-1: 1-sqrt(2-r1);
        double r2 = 2*curand_uniform(&states[t]), dy=r2<1 ? sqrt(r2)-1: 1-sqrt(2-r2);

        Vec d = cx * (((sx+.5 + dx)/2 + x)/w - .5) + cy*(((sy+.5 + dy)/2 + y)/h - .5) + cam.d;
        d = d.norm();
        Vec res = linear_radiance(new_ray(cam.o+d*140,d),0, &states[t]);
    ...
    if (threadIdx.x < BLOCK_SIZE) {
        out[i] = out[i] + temp_res[threadIdx.x*4] + temp_res[threadIdx.x*4+1] + temp_res[threadIdx.x*4+2] + temp_res[threadIdx.x*4+3];
    }
}

int main(int argc, char *argv[]) {
    //float BLOCK_SIZE_X = 16;
    //float BLOCK_SIZE_Y = 8;
    ...
    calc_pixel<<<grid, block>>>(device_out, samps, cam, cx, cy);
