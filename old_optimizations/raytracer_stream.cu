...
int main(int argc, char *argv[]) {
    ...
    /* int streamSize = sizeof(Vec) * w * h / num_streams; */
    /* for (int i = 0; i < num_streams; ++i) */
    /* { */
    /*     int offset = i * w * h / (num_streams); */
    /*     cudaMemcpyToSymbolAsync(SPHERES, spheres, NUM_SPHERES * sizeof(Sphere), 0, cudaMemcpyHostToDevice, stream[i]); */
    /*     calc_pixel<<<grid, block, 0, stream[i]>>>(device_out, samps, offset); */
    /*     cudaMemcpyAsync(&host_out[offset], &device_out[offset], streamSize, cudaMemcpyDeviceToHost, stream[i]); */
    /*     for (int j=offset; j<(i+1)*(w*h/num_streams); j++) */
    /* 	   fprintf(f,"%d %d %d ", toInt(host_out[j].x), toInt(host_out[j].y), toInt(host_out[j].z)); */
    /* } */
    /* for (int i = 0; i < num_streams; ++i) */
